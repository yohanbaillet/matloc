...
export default function EndothermicTech() {
  ...
  return (
    <section className="theme-light section-v2 relative overflow-hidden bg-background text-foreground">
      <div className="container-v2 relative">
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-12 lg:gap-20">
          <div className="lg:col-span-7 flex flex-col gap-12">
            <div className="flex flex-col gap-4 max-w-2xl">
              <TechLabel>{t('endothermic_label')}</TechLabel>
              <h2 className="display-lg text-foreground">{t('endothermic_title')}</h2>
            ...
            <ul className="flex flex-col gap-3 border-t border-border pt-8">
              {bullets.map((b, i) => (
                <li key={i} className="flex items-start gap-4 text-base text-muted-foreground">
